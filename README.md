# yadmm - Yet Another Diva Mod Manager
A (currently barebones) mod manager for Project Diva MegaMix+ written in Electron and React.
## Currently Supported Features:
- literally nothing except rendering example elements.
## Planned Additions
- Ability to enable/disable mods
- Ability to uninstall mods, and install from various services
- Ability to check for mod updates
- Ability to update/install DivaModManager
- Ability to reorder mod priority
- Probably more that I'm forgetting